...
theta = 0.5  # threshold to find rare nodes
it = 10
psize = 10
...
# Reading the input length from given file
...
def fitnessindi(y):
    ...
    a, b, c = logic(golden_circuit_file, y)
    e, f, g = logic(trojan_inserted_file, y)
    count=0
...
def total_fitness(y):
    ...
    b = fitness_score(y)
    fit = (1/np.e**(-2*a))*b
        #fit.append((1/np.e**(-40*a))*b)
    #print(a,b,fit)
    return fit

'''
-----------------------------------------------------------------
Pattern Generation using Diffrential Evolution
-----------------------------------------------------------------
'''
...
def differential_algorithm(pop_len=n, crossp=0.8, popsize=psize, its=it):
...
detection  part
-----------------------------------------------------------------
The Trigger coverage and Trojan coverage are calculated and compared for each optimized pattern
'''

flag = 0

for i in range(len(pop)):
    print('Pattern :', pop[i], '\nFitness ', total_fitness(pop[i]))
    ...
    for j in range(len(j_2)):
        for k in range(len(rare_node_3)):
            if j_2[j][0] == rare_node_3[k]:
                if j_2[j][1] != m_2[j][1]:
                    count_2 += 1
    print('Trigger coverage for benchmark circuit:', count_1/len(rare_node_2), '\nTrigger coverage for given circuit:', count_2/len(rare_node_2))
    print('Number of output changed by trojan circuit represented as Trojan coverage:', count / len(b_1))
    print()
    if count > 0 and count_2/len(rare_node_2) != count_1/len(rare_node_2):
        flag = 1

print('Rare Nodes :',rare_node_3)
if flag == 1:
