...
def fitness(pop):
    ...
    for i in range(len(pop)):
        ...
        #scp = scoap(trojan_inserted_file)
        #rank = []
        #for q in range(len(scp)):
         #   rank.append([scp[q][0], calc_rank(scp[q], trn_pr[q], g[q])])
        count = 0
        for j in range(len(g)):
            for k in range(len(rare_node)):
                if g[j][0] == rare_node[k]:
                    if g[j][1] != c[j][1]:
                        count += 1
...
def differential_algorithm(pop_len=n, crossp=0.8, popsize=psize, its=it):
    ...
    while i < its:
        ...
        if fitness(trail_pop) > fitness(pop):
            pop[:] = trail_pop[:]
        i += 1
    return pop
